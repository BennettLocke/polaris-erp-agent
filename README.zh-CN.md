...
是 GitHub 小程序仓库的本地克隆。之前会看到两个目录，是因为 `sj-mall-uniapp` 是实际业务源码目录，`polaris-ai-erp-weapp` 是仓库名对齐的 Git 克隆。准确源码已经同步到 GitHub 小程序仓库，后续交接、发布和回滚应以该仓库为准。服务端部署不依赖小程序源码；小程序发布时，应从小程序仓库用 uni-app / HBuilderX 构建，并指向生产后端域名。

小程序基于 uni-app + Vue 3，负责首页、分类、商品列表、商品详情、订单查询、销售单入口、登录/客户绑定、设置、联系咨询和分享资源等客户侧体验。它不直连数据库，而是通过 `src/api/*.js` 和 `src/config.js` 调用 sjagent 服务层，主要消费 `/api/mini/*` 以及相关小程序接口。商品、客户、订单、库存、上下架、热销统计和安全鉴权仍由本仓库服务端统一负责。
