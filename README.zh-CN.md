...
这个仓库是主服务端。小程序前端源码和小星设备端源码都不直接放在这个仓库里：当前准确的小程序源码在本地 `sj-mall-uniapp` 工作区，小星设备端源码独立维护在 [BennettLocke/polaris-xiaoxing-device](https://github.com/BennettLocke/polaris-xiaoxing-device)。
...
这个目录是现在应当认准的商城小程序代码，基于 uni-app + Vue 3，负责首页、分类、商品列表、商品详情、订单查询、销售单入口、登录/客户绑定、设置、联系咨询和分享资源等客户侧体验。它不直连数据库，而是通过 `src/api/*.js` 和 `src/config.js` 调用 sjagent 服务层，主要消费 `/api/mini/*` 以及相关小程序接口。商品、客户、订单、库存、上下架、热销统计和安全鉴权仍由本仓库服务端统一负责。
...
这个不是当前小程序的准版本，后续交接、排查和发布不要优先使用它，除非项目负责人明确确认它重新成为主线。服务端部署不依赖小程序源码；小程序发布时，应从 `sj-mall-uniapp` 用 uni-app / HBuilderX 构建，并指向生产后端域名。
...
这个本地组件库不属于服务端部署必需文件，但它是 Polaris / 肆计小程序和后台视觉规范的本地源头。它维护三套内容：
...
- 本地组件库负责沉淀小程序和网页预览组件，保持 `sj-` 前缀、中文文案、组件状态、示例数量和验收规则统一。
- 新增或重做组件时，先在本地组件库验证 CSS / uni-app / preview，再把成熟的视觉和交互规则同步到主仓库后台页面。
...
- **小程序协同**：本地组件库的 `uni-app-components/sj-*` 能让小程序页面和后台业务视觉保持一致。
