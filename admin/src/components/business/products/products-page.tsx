    ...
    <>
      <Card size="sm" className="product-spu-card">
        ...
        <CardHeader>
          ...
          <CardAction className="product-spu-badges">
            <Badge variant={isListed ? "secondary" : "outline"}>
              {isListed ? "已上架" : "未上架"}
            </Badge>
            {Number(product.is_one_case_purchase || 0) ? <Badge variant="outline">1件起订</Badge> : null}
          </CardAction>
        </CardHeader>
        <CardContent>
          <div className="product-spu-summary-row">
            <strong className="product-spu-price">{productPriceText(product)}</strong>
            <span>{productColorCount(product)} 色</span>
            <span>{productStockText(product)}</span>
          </div>
          <div className="product-spu-detail-row">
            <span>件规：{productPieceText(product)}</span>
          </div>
          <div className="product-spu-colors" aria-label={`颜色：${productColorsText(product)}`}>
            {colors.slice(0, 3).map((color) => (
              <Badge key={color} variant="ghost">{color}</Badge>
            ))}
            {colors.length > 3 ? <Badge variant="outline">+{colors.length - 3}</Badge> : null}
          </div>
        </CardContent>
