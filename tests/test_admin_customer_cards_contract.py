...
class AdminCustomerCardsContractTest(unittest.TestCase):
    ...
    def test_customer_page_uses_shadcn_customer_components(self):
        ...
        self.assertIn("共计：", service_source)
        self.assertIn("statement.get('sales_count')", service_source)
        self.assertIn("statement.get('sales_quantity')", service_source)
        self.assertIn("statement.get('sales_amount')", service_source)
        ...
        self.assertIn("api.customers(query, 80, { signal })", customers_page)
