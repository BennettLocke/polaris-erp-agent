...
日期: 2026-05-26  
适用入口: `/admin/settings`  
相关旧入口: `/admin/media`, `/admin/miniapp-images`, 旧 Web `/web` 设置区和图片资产区  
保护范围: 旧 Web `/web` 不在本轮改造范围内，不能影响服务器现有 `/web` 使用。
...
- 其他设置功能参考旧 Web，保留编号、商品基础、库存规则、收款结款、图片/OSS、用户权限、打印设置等已有业务逻辑。
...
第一阶段 React 设置页只承接旧 Web 已稳定可用、当前 React 已有接口支撑的功能。小程序完整设计器、仓库增删改、ASR 热词管理可以在设置页预留入口，但不要在本轮伪造功能。
