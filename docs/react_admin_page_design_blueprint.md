...
日期：2026-05-24
范围：`/admin` React 后台
当前状态：旧 `/web` 页面已下线，视觉规范只面向 `/admin` React 后台。
...
- 新建/编辑商品的小表单版本。
- 打印设置预览。
- 商品、客户、订单的详情弹窗。当前后台统一优先使用中间 Dialog，避免右侧 Sheet 造成信息被挤压。
...
- 加入明细。
- 不提交真实单时，至少验证表单状态和合计。
- 打开 `http://127.0.0.1:8081/admin`，确认后台主入口仍可用。
...
| 顺序 | 页面 | 重点 |
| --- | --- | --- |
| 1 | 工作台 | AI 对话、结构化确认、图片识别、最近业务记录、中间弹窗 |
| 2 | 开单 | 快速开单、客户选择、商品明细、提交结果、详情弹窗 |
| 3 | 销售单 | 紧凑列表、详情 Dialog、打印、删除回滚展示 |
| 4 | 客户 | 客户卡片、余额、月结、收款/充值/结款、客户详情 |
| 5 | 商品 | SPU 列表、颜色、件规、1 件起、中间编辑弹窗、图片选择 |
| 6 | 图片资产 | SPU 聚合、分类、绑定、上传、删除、分页和懒加载 |
| 7 | 库存 | 明细、日志、出入库、盘点、调拨、仓库设置 |
| 8 | 订单 | 设计稿/制作/配送/丝印状态、操作人、销售单关联 |
| 9 | 设置 | 编号、商品基础、库存规则、收款结款、图片/OSS、用户权限、打印 |

工作台详细执行标准见：`docs/react_admin_workbench_page_development_handbook.md`。
