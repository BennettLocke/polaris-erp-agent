...
| 工作流订单 | `/admin/workflow` | `/api/workflow/orders`, `/api/workflow/orders/<id>/status`, `DELETE /api/workflow/orders/<id>` | `WorkflowService` | 已可用 |
...
| --- | --- | --- | --- | --- | --- |
| 首页数字 | GET | `/api/dashboard/summary` | 无 | 今日销售、今日金额、待处理工作流 | `DashboardService.summary()` |
| 最近业务记录 | GET | `/api/orders/recent` | `limit` | 右侧最近销售单和工作流单 | `DashboardService.recent_orders()` |
...
| 商品分类 | GET | `/api/product/categories` | 无 | 一级/二级分类筛选 | `ProductService.categories()` |
...
| 动作 | 方法 | 接口 | 参数 | 用途 | Service |
| --- | --- | --- | --- | --- | --- |
| 工作流列表 | GET | `/api/workflow/orders` | `keyword`, `page`, `page_size`, `status` | 工作流订单列表 | `WorkflowService.list_orders()` |
| 创建/编辑工作流 | POST | `/api/workflow/orders` | `customer_name`, `goods_name`, `order_quantity`, `color`, `order_images`, `remark` | 保存工作流订单 | `WorkflowService.save_order()` |
| 状态更新 | POST | `/api/workflow/orders/<id>/status` | `field`, `value` | 制作/配送/丝印状态 | `WorkflowService.update_status()` |
| 删除工作流 | DELETE | `/api/workflow/orders/<id>` | 无 | 删除工作流订单 | `WorkflowService.delete_orders()` |
| 工作流图片上传 | POST | `/api/workflow/images/upload` | `image` | 图片识别/工作流附件 | 上传服务 |

前端要求：

- 工作流不扣库存。
- 工作流转销售单必须走销售单服务，不由前端直接拼数据写库。
...
| 库存规则 | GET/POST | `/api/settings/system/inventory_rules` | 默认仓库、负库存开关、只读固定规则 | `SettingsService` |
